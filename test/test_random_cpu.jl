...
@testset "CthulhuVision.Random CPU" begin
    ...
    @testset "Xoshiro256++ validation" begin
        ...
        xshr1 = Xoshiro256pp(0xe220a8397b1dcdaf, 0x6e789e6aa1b965f4, 0x06c45d188009454f, 0xf88bb8a8724c81ec)

        for i = 1:length(xshr256_test_1)
            @test next(xshr1) == xshr256_test_1[i]
        end
        ...
        xshr2 = Xoshiro256pp(0xa747f481346acb72, 0x8ad918349ab73966, 0xbec62b7cc97a0873, 0xf4975b3f04f272de)

        for i = 1:length(xshr256_test_2)
            @test next(xshr2) == xshr256_test_2[i]
        end
    end
end